...
:::note
- `/clear` and `/btw` exist in the source tree.
- No built-in `/compact` command was found in the current `main` branch. The related command is `/context`, which displays context usage and compression progress.
:::
...
No built-in `/compact` command was found in the current `main` branch. To inspect context compression-related status, use `/context`.
